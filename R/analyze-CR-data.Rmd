...
denials_by_court <- dat %>%
  group_by(court_std, granted_denied, .drop = FALSE) %>%
  summarise(n_cases_by_court_status = n()) %>%
  filter(granted_denied == "denied") %>%
  left_join(group_counts, by = "court_std") %>%
  mutate(court_deny_rate = n_cases_by_court_status / n_documents_by_court) %>%
  dplyr::select(court_std, n_documents_by_court, court_deny_rate)

denials_by_judge <- dat %>%
  ...
  mutate(judge_deny_rate = n_cases_by_judge_status / n_cases_by_judge) %>%
  filter(granted_denied == "denied") %>%
  arrange(desc(n_cases_by_judge)) %>%
  left_join(denials_by_court, by = "court_std") %>%
  mutate(diff_judge_court = court_deny_rate - judge_deny_rate) 

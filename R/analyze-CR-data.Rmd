...
ucla_dat <- read_xlsx(file.path(dat_path, "2020-04-08-master.xlsx")) %>%
...
#write_csv(dat, file.path(dat_path, "CR_data.csv"), na = "")
...
map_dat <- sf::read_sf(file.path("~", "UCLA", "misc-data", "federal courts", "US_District_Court_Jurisdictions.json")) %>%
  select(STATEFP, NAME, STATE, CHIEF_JUDG, NOMINATING, TERM_AS_CH, DISTRICT_N, geometry)
...
crim_dat_out <- crim_dat_cln %>%
  ...
  select(District, Total) %>% ## only keep totals for now 
...
dat <- dat_merged %>%
  ...
  rename(case_name = case_name_coalesced,
       ...
       doc_title = Title,
       def_last_name = def_last_name_coalesced,
...
tab_2 %>%
  select(-c(court_percent, ntl_percent, diff)) %>%
